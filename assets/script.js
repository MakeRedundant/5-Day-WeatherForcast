...
const clearAllButton = document.getElementById("clearRecentBtn");
...
const apiCall =
  "api.openweathermap.org/data/2.5/forecast?lat={lat}&lon={lon}&appid={f62c9d6db823b3bd186bc980988733d1}";
...
function getWeatherData(city) {
  ...
  const geocodingUrl =
    /*base url*/ "https://api.openweathermap.org/geo/1.0/direct?q=" /*query parameters */ +
    city +
    ",limit=1&appid=" +
    apiKey; //limit one to return the most releveant result + apikey to authenicate the request

  // Make the API call to fetch the latitude and longitude
  // makes a fetch request to the geocodingUrl we constructed
  fetch(geocodingUrl)
    .then(function (response) {
      //then() method to handle response when it is successfully recevied.
      if (!response.ok) {
    ...
    .then(function (data) {
      //another then() method to handle the parsed JSON data
      if (data.length === 0) {
      ...
      const apiUrl =
        ...
        longitude +
        "&units=metric&appid=" +
        apiKey; //query parameters of lat+ lon retrives metric data
      ...
      fetch(apiUrl)
        ...
        .then(function (data) {
          ...
          if (data.list[0].weather[0].icon) {
            ...
            // Show the image
            document.getElementById("currentIcon").style.display =
              "inline-block";
          }

          //Day +1
          document.querySelector("#icon-1").src =
            "https://openweathermap.org/img/wn/" +
            data.list[8].weather[0].icon +
            ".png";
          document.querySelector("#temp-1").textContent =
            "Temp: " + data.list[8].main.temp + " ºC";
          document.querySelector("#wind-1").textContent =
            "Wind: " + data.list[8].wind.speed + " m/s";
          document.querySelector("#humidity-1").textContent =
            "Humidity: " + data.list[8].main.humidity + " %";
          document.getElementById("description-1").textContent =
            "Description: " + data.list[8].weather[0].description;

          //Day +2
          document.querySelector("#icon-2").src =
            "https://openweathermap.org/img/wn/" +
            data.list[16].weather[0].icon +
            ".png";
          document.querySelector("#temp-2").textContent =
            "Temp: " + data.list[16].main.temp + " ºC";
          document.querySelector("#wind-2").textContent =
            "Wind: " + data.list[16].wind.speed + " m/s";
          document.querySelector("#humidity-2").textContent =
            "Humidity: " + data.list[16].main.humidity + " %";
          document.getElementById("description-2").textContent =
            "Description: " + data.list[16].weather[0].description;

          //Day +3
          document.querySelector("#icon-3").src =
            "https://openweathermap.org/img/wn/" +
            data.list[24].weather[0].icon +
            ".png";
          document.querySelector("#temp-3").textContent =
            "Temp: " + data.list[24].main.temp + " ºC";
          document.querySelector("#wind-3").textContent =
            "Wind: " + data.list[24].wind.speed + " m/s";
          document.querySelector("#humidity-3").textContent =
            "Humidity: " + data.list[24].main.humidity + " %";
          document.getElementById("description-3").textContent =
            "Description: " + data.list[24].weather[0].description;

          //Day +4
          document.querySelector("#icon-4").src =
            "https://openweathermap.org/img/wn/" +
            data.list[32].weather[0].icon +
            ".png";
          document.querySelector("#temp-4").textContent =
            "Temp: " + data.list[32].main.temp + " ºC";
          document.querySelector("#wind-4").textContent =
            "Wind: " + data.list[32].wind.speed + " m/s";
          document.querySelector("#humidity-4").textContent =
            "Humidity: " + data.list[32].main.humidity + " %";
          document.getElementById("description-4").textContent =
            "Description: " + data.list[32].weather[0].description;

          //Day +5
          document.querySelector("#icon-5").src =
            "https://openweathermap.org/img/wn/" +
            data.list[39].weather[0].icon +
            ".png";
          document.querySelector("#temp-5").textContent =
            "Temp: " + data.list[39].main.temp + " ºC";
          document.querySelector("#wind-5").textContent =
            "Wind: " + data.list[39].wind.speed + " m/s";
          document.querySelector("#humidity-5").textContent =
            "Humidity: " + data.list[39].main.humidity + " %";
          document.getElementById("description-5").textContent =
...
// +1 Day
document.addEventListener("DOMContentLoaded", function () {
  document.getElementById("date-plus-1").textContent = dayjs()
    .add(1, "day")
...
// +2 Day
document.addEventListener("DOMContentLoaded", function () {
  document.getElementById("date-plus-2").textContent = dayjs()
    .add(2, "day")
    .format("dddd DD/MM/YY");
});
...
// +3 Day
document.addEventListener("DOMContentLoaded", function () {
  document.getElementById("date-plus-3").textContent = dayjs()
    .add(3, "day")
    .format("dddd DD/MM/YY");
});
...
// +4 Day
document.addEventListener("DOMContentLoaded", function () {
  document.getElementById("date-plus-4").textContent = dayjs()
    .add(4, "day")
    .format("dddd DD/MM/YY");
});
...
// +5 Day
document.addEventListener("DOMContentLoaded", function () {
  document.getElementById("date-plus-5").textContent = dayjs()
    .add(5, "day")
    .format("dddd DD/MM/YY");
});

//Hides the placeholder image for the icons
document.getElementById("currentIcon").src = "";
...
clearAllButton.addEventListener("click", function () {
  ...
  // Clear the recent searches displayed on the page
  var recentSearchesContainer = document.getElementById("recentSearches");
  recentSearchesContainer.innerHTML = "";
});
